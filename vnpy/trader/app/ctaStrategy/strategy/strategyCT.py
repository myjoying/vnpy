...
class CentralBaseSet:
    ...
    def setDBInfo(self, nodedbName, cbdbName, dbSymbol):
        ...
        #self.engine.removeCBData(self.nodedbName, self.dbSymbol, 'datetime')
        #self.engine.removeCBData(self.cbdbName, self.dbSymbol, 'start')
            ...
            if up_flag and go_judge:
                ...
                if close_price < self.node_list[-1].value:
                    ...
                    if len(self.node_list)>0:
                        self.engine.insertCBData(self.nodedbName, self.dbSymbol, self.node_list[-1])
    ...
    def get_Centralbase_Step(self):
        ...
        if len(self.centralbase_list)>0:
            self.engine.insertCBData(self.cbdbName, self.dbSymbol, self.centralbase_list[-1])        
    ...
    def getBeichi_LastTwo_Step(self, isMinOrNot = True):
        ...
        if isbeichi:
            ...
            #背驰节点记入数据库
            self.engine.insertCBData(self.nodedbName, self.dbSymbol, self.beichi_list[-1])
    ...
    def getBeichi_Share_With_LowBeichi_Step(self, low_beichi):
        ...
        if isbeichi:
            ...
            #背驰节点记入数据库
            self.engine.insertCBData(self.nodedbName, self.dbSymbol, self.share_beichi_list[-1]) 
        
        if isbeichi_pz:
                
            #背驰节点记入数据库
            self.engine.insertCBData(self.nodedbName, self.dbSymbol, self.share_beichi_pz_list[-1]) 
    ...
    def getBeichi_PanZheng_step(self):
        ...
        if isbeichi:
            #背驰节点记入数据库
            self.engine.insertCBData(self.nodedbName, self.dbSymbol, self.beichi_pz_list[-1]) 
